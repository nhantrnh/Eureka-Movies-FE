...
export default function Movies() {
    ...
    let maxPerPage = 20;
    const { genreIdOrCategoryName, searchQuery, type, generalId } = useSelector((state) => state.curruntGenreOrCategory);
    if (genreIdOrCategoryName === 'TrendingDay') {
        maxPerPage = 10;
    }
    const { data, error, isLoading, isFetching } = useGetMoviesQuery({ genreIdOrCategoryName, page, searchQuery, maxPerPage, generalId, type });
    ...
    return <>
        <div>
            ...
            <Pagination curruntPage={page} setPage={setPage} totalPages={type === 'LLM' ? 1 : data?.paging?.totalPage} />
        </div>
    </>
}
